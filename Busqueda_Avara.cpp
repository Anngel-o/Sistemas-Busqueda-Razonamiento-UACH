...
// Esta función busca el camino desde el nodo inicial hasta el nodo meta
void busqueda_avara(char inicio, char meta) {
    // Cola de prioridad para nodos a explorar, ordenada por el costo heurístico (h)
    ...
    // Añadir el nodo inicial al conjunto abierto con heurística calculada
    conjunto_abierto.emplace(inicio, heuristica(inicio, meta));
    // Almacenar el nodo inicial en el mapa de todos los nodos
    todos_nodos[inicio] = new Nodo(inicio, heuristica(inicio, meta));

    // Bucle principal de búsqueda
    while (!conjunto_abierto.empty()) {
        // Obtener el nodo con el menor costo heurístico (h) del conjunto abierto
        ...
        for (char vecino : grafo[actual.estado]) {
            ...
            // Crear un nuevo nodo vecino con el costo heurístico calculado y el nodo actual como padre
            Nodo* nodo_vecino = new Nodo(vecino, costo_h, new Nodo(actual));
...
int main() {
    ...
    // Llamar a la función de búsqueda avara con los estados proporcionados por el usuario
